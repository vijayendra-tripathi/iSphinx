...
public class Decoder {
    ...
    open func processRaw(data: NSData) -> Int {
        let dataLenght = data.length / 2
        return Int(ps_process_raw(pointer, data.bytes.assumingMemoryBound(to: int16.self), dataLenght, 0, 0))
    }
    ...
    internal func getHypotesis(isFinal: Bool) -> Hypothesis {
        return Hypothesis(decoder: self, isFinal: isFinal)
    }
